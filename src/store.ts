 ...
 * さらに、半信頼な入力（ウィンドウタイトルはブラウザのタブ名などで外部から影響を受け得る）が
 * 管理マーカー `<!-- timemeter:sessions:... -->` を含んでいた場合に、パースが誤ってブロックを
 * 途中で閉じて当日データを破損させるのを防ぐため、HTMLコメント開始 `<!--` を無害化する
 * （ゼロ幅スペースを挿入。両マーカーとも `<!--` で始まるのでこれで完全一致を防げる。
 *  一度無害化すると `<!--` を含まなくなるので冪等・ラウンドトリップ安全）。 */
const ZWSP = "\u200b"; // ゼロ幅スペース（不可視文字を直書きせず明示エスケープで定義）

function escapeCell(raw: string): string {
	return raw
		.replace(/<!--/g, `<!${ZWSP}--`) // マーカー衝突防止（<!-- を分断して完全一致を防ぐ）
		.replace(/\|/g, "\\|")
		.replace(/\r?\n/g, "<br>");
}
 ...
 * アプリ名は YAML の二重引用符キーにするため、`\`・`"` をエスケープし改行は空白に潰す
 * （半信頼なアプリ名が `"` や改行を含んでも frontmatter 全体を壊さないようにする）。 */
function buildManagedFmLines(date: string, totalMin: number, totals: Map<string, number>): string[] {
	...
	for (const [app, min] of totals) {
		const key = app.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/[\r\n]+/g, " ");
		lines.push(`  "${key}": ${min}`);
	}
	return lines;
}
