...
- 合計バーのアプリをクリック → モーダルで「何をしていたか」を入力 → **「追記先」ドロップダウン**（バーの下・先頭=デイリー／その下に最近使ったファイル）で選んだファイルに `- アプリ: 内容` を追記。デイリーは `## ✅ やったこと` セクションに、それ以外のファイルはファイル末尾に入る。デイリーのフォルダ/日付書式は Obsidian コアの「デイリーノート」設定に追随（未設定時は `デイリー`/`YYYY-MM-DD (ddd)`）。対象ファイルが無ければ通知のみ
...
- [ ] 合計バーのアプリをクリック→モーダルで入力→追加で「追記先」で選んだファイルに `- アプリ: 内容` が入る（デイリー=やったこと欄／他=末尾）。追記先ドロップダウンにデイリー＋最近使ったファイルが出る／デイリーのパスがコア設定に追随する
- [ ] 手動ログ追加（デスクトップ＆モバイル）
- [ ] ラップで区切りが入る
